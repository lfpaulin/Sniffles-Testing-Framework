...
def get_arguments():
    ...
    subparser_bench.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')
    subparser_bench.add_argument('-s', '--sleep', type=int, required=False, dest='sleep_sec', default=2, help='Wait time between job submissions')

    # ############################################################################################ #
    # Single sample benchmark with GIAB HG002 v0.6 (GRCh37)
    giab_help = "Single sample benchmark with GIAB HG002 v0.6 (GRCh37)"
    subparser_giab = subparsers.add_parser("giab", help=giab_help)
    subparser_giab.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')

    # ############################################################################################ #
    # Single sample benchmark with GIAB HG002 CMRG (GRCh38)
    cmrg_help = "Single sample benchmark with GIAB HG002 CMRG (GRCh38)"
    subparser_cmrg = subparsers.add_parser("cmrg", help=cmrg_help)
    subparser_cmrg.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')

    # ############################################################################################ #
    # Trio/Mendelian (BCFtools required)
    mendelian_help = "Trio/Mendelian bench"
    subparser_mendelian = subparsers.add_parser("mendelian", help=mendelian_help)
    subparser_mendelian.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')

    # ############################################################################################ #
    # Merge benchmark
    population_help = "Test merging procedure"
    subparser_merge = subparsers.add_parser("merge", help=population_help)
    subparser_merge.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')
    ...
    subparser_popmerge.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')
    ...
    subparser_mosaic.add_argument('-j', '--json', type=str, required=True, dest='json', default="", help = 'JSON input with all parameters')
